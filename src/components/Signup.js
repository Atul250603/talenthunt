...
function Signup({signupDisplay,setsignupDisplay}){
    const [view, setview] = useState(false);
    return(
        <div className={`fixed top-0 right-0 h-screen px-2 shadow-2xl flex items-center w-1/4 bg-white z-50 slide ${(signupDisplay)?"slideIn":"slideOut"}`}>
            ...
            <div className="w-full px-3 ">
                ...
                <div className="border-2 border-purple-600 rounded px-3 py-1 w-full mt-5" >
                    <div className="text-purple-600">Confirm Password</div>
                    <div className="py-1 w-full flex items-center"><input type={`${(!view)?"password":"text"}`} name="cpassword" id="cpassword" className="outline-none w-full" autocomplete="off"/>{(!view)?<img src={viewIcon} alt="view" onClick={()=>{setview(true)}} className='hover:cursor-pointer'/>:<img src={hideIcon} alt="hide" onClick={()=>{setview(false)}} className='hover:cursor-pointer'/>}</div>
                </div>
                <div className="text-center mt-6">
                <button className="text-white rounded bg-purple-600 px-3 py-3 w-full">Signup</button>
                </div>
            </div>
            </div>
        </div>
